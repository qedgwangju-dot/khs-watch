...
class GlobalRatesFreshnessGuardTest(unittest.TestCase):
    ...
    def test_live_fx_replaces_daily_fred_for_current_signal(self):
        ...
        self.assertEqual(pending["last_values"]["usdjpy_daily_change_pct"], -0.60)
        self.assertEqual(freshness["fred_usdjpy_reference"], 159.97)
        self.assertNotIn("usdjpy", [e["metric"] for e in alert["events"]])
    ...
    def test_same_date_spread_remains_eligible(self):
        pending, alert = self.base()
    ...
    def test_report_explains_source_time_and_withheld_spread(self):
        text = "\n".join([
            "[글로벌 금리·엔캐리 경보] 🟢",
            "판정: 관찰",
        ...
        freshness = {
            ...
            "live_fx_signal_eligible": True,
            "live_fx_price": 158.25,
            "live_fx_change_pct": -0.60,
            "live_fx_timestamp_utc": "2026-09-01T09:00:00Z",
            "live_fx_age_seconds": 45.0,
        }
        out = annotate_report(text, freshness)
        self.assertIn("기준일 불일치 — 계산 보류", out)
        self.assertIn("USD/JPY 158.250", out)
        self.assertIn("2026-09-01 18:00:00 KST", out)
