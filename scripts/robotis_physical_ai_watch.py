...
def score(item: dict) -> int:
    ...
    if NOISE.search(text):
        return -20
    s = 0
    # Entity gate: avoid generic robotics noise.
    ...
    if item.get("source") in TRUSTED:
        s += 2
    # Highest-value transition events.
...
def risk(cat: str) -> str:
    ...
    if cat == "리서치·재평가":
        return "증권사 탑재가치는 실제 OEM 납품단가가 아니며 액추에이터 ASP 상승과 혼동하면 안 됩니다."
...
def main() -> None:
    ...
    # Only recent articles; avoid resurfacing old history.
    ...
    # Mark all currently relevant items seen so duplicates remain silent.
    for x in recent:
        seen.add(x["key"])
    # Keep bounded state.
    ...
    lines = []
    if selected:
        lines.append("🚨 로보티즈·피지컬AI 구조 변화 감지")
        lines.append("")
        for i, x in enumerate(selected, 1):
            ...
            lines.append(f"• 출처: {x.get('source') or '미상'}" + (f" | {when} KST" if when else ""))
            lines.append(f"• 단계: {cat}")
            lines.append(f"• 의미: {meaning(cat)}")
            lines.append(f"• 역풍: {risk(cat)}")
            lines.append(f"• 원문: {x['link']}")
            lines.append("")
        lines.append("판정 기준: 신규 수주·고객 실명·양산/출하·생산능력·현장 배치·대당 액추에이터 탑재가치·기관 지분 변화처럼 돈 버는 능력/수급/시간표를 바꾸는 내용만 알림. 단순 주가 기사·테마 반복은 제외.")
