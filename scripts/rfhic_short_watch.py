...
def extract_latest():
    ...
    v = volume.iloc[-1]
    v_date = to_date_str(volume.index[-1])

    # pykrx fields: 공매도, 매수(전체 거래량), 비중
    short_volume = int(v.get("공매도", v.iloc[0]))
    total_volume = int(v.get("매수", v.iloc[1] if len(v) > 1 else 0))
    short_ratio = float(v.get("비중", (short_volume / total_volume * 100 if total_volume else 0)))

    b = balance.iloc[-1]
    b_date = to_date_str(balance.index[-1])
    short_balance = int(b.get("공매도잔고", b.iloc[0]))
    listed_shares = int(b.get("상장주식수", b.iloc[1] if len(b) > 1 else 0))
    balance_value = float(b.get("공매도금액", b.iloc[2] if len(b) > 2 else 0))
    market_cap = float(b.get("시가총액", b.iloc[3] if len(b) > 3 else 0))
    balance_ratio = float(b.get("비중", (short_balance / listed_shares * 100 if listed_shares else 0)))

    prev_balance = None
    prev_balance_date = None
    if len(balance) >= 2:
        p = balance.iloc[-2]
        prev_balance = int(p.get("공매도잔고", p.iloc[0]))
        prev_balance_date = to_date_str(balance.index[-2])
...
def make_alert(d):
    ...
    # Simple interpretation: volume is today's pressure, balance is the accumulated reported position.
    if delta is None:
        interpretation = "순보유잔고의 이전 비교값이 없어 방향 판단을 보류합니다."
    elif delta > 0:
        interpretation = "당일 공매도와 별개로, 신고대상 순보유잔고도 늘어 실제 숏 포지션이 최근 더 쌓인 방향입니다."
    elif delta < 0:
        interpretation = "당일 공매도 거래가 있어도 신고대상 순보유잔고는 줄어, 누적 숏 포지션은 최근 감소한 방향입니다."
    else:
...
def main():
    ...
    # One Telegram report per newly published KRX trading date.
    ...
    if rebaseline:
        # Do not send old data as a market alert; workflow sends a one-time connection confirmation instead.
        ...
        # State is finalized only after Telegram delivery is confirmed by the workflow.
        pending["last_sent_volume_date"] = data["volume_date"]
