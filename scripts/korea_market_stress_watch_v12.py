...
def _append_cosmetics_fx_context() -> None:
    ...
    if direction == "down":
        ...
        market_flow = "• 시장 흐름: 조정장에서도 강세를 보이던 대표 수출주 화장품 ETF가 <b>원/달러 1,550원대 → 1,350원대 급락</b> 뒤 실적 악화 우려로 급격한 조정을 받음"
    else:
        verdict = "원화 약세 → 달러 수출 매출의 원화 환산액 증가 → 수출 비중 높은 화장품 기업의 원화 환산 실적에 우호적"
        impact = "🔺 <b>환율 상승 시 원화 환산 실적 수혜 가능 업종</b>"
        market_flow = "• 시장 흐름: 원/달러 반등은 최근 환율 급락으로 눌렸던 대표 수출주 화장품 ETF의 원화 환산 실적 우려를 완화할 수 있음"
...
def _append_auto_fx_timing() -> None:
    ...
    additions = [
        ...
        "• 4분기: 환율이 <b>1,350원 수준에 머물면</b> 충당금 감소라는 방어 효과가 사라져 수출 수익성 하락이 실적에 더 온전히 반영될 수 있음",
        ...
        "• 따라서 알림 해석은 <b>당일 주가 반응</b>과 <b>4분기 실적 반영 시점</b>을 분리",
