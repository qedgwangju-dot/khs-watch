...
def enforce_semiconductor_cycle_contract() -> None:
    ...
    def classify(row: dict, now):
        ...
        is_grid_policy = any(base.has(text, term) for term in grid_policy_terms) and any(base.has(text, term) for term in ["approval", "regulatory", "permitting", "delay", "interconnection", "commission", "fe…46536 tokens truncated…ormuz_escalation"):
