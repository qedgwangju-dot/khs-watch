...
def cta_context() -> dict:
    ...
    if composite:
        verdict = "CTA 숏커버 복합 확인"
    elif short_bias and rising:
        verdict = "CTA 숏 포지션은 남아 있고 국채선물 반등이 일부 확인 — 청산 후보, 아직 확정 아님"
    elif short_bias and falling:
        verdict = "CTA 숏 편향은 남아 있으나 국채선물이 하락 — 숏커버 확인 안 됨"
    elif short_bias:
        verdict = "CTA 숏 편향은 남아 있으나 가격 확인 부족 — 숏커버 확정 전"
    else:
        verdict = "CTA 숏 편향 자체가 약화 — 숏커버 촉매 민감도 낮음"

    return {
        "last_checked_kst": str(state.get("last_checked_kst") or "확인 불가"),
...
def failure_lines(exe: dict, causal: dict, cta: dict) -> list[str]:
    ...
    if not cta.get("composite_confirmed"):
...
def build_block(exe: dict, causal: dict, cta: dict) -> str:
    ...
    return "\n".join(
        [
            ...
            f"• ④ CTA 반응: {cta.get('verdict')} · {futures_text}",
            f"• 실패 조건: {failures}",
