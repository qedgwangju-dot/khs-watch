#!/usr/bin/env python3
"""GAMEJOA policy/regulatory high-impact watch.
...
SEC_USER_AGENT = os.getenv(
    "SEC_USER_AGENT",
    "GAMEJOA-policy-watch/0.2 contact=please-set-SEC_USER_AGENT",
)
...
STAGE_KEYWORDS = {
    ...
    "fda_decision": ["fda approves", "complete response letter", "crl", "approval", "rejection", "승인", "거절"],
}
...
SECTOR_KEYWORDS = {
    ...
    "바이오/FDA": ["fda", "clinical", "drug", "approval", "crl"],
...
def parse_courtlistener(text: str, source: Source) -> list[dict]:
    ...
    for row in (data.get("results") or [])[:10]:
        ...
        items.append(
            {
                "source": source.name,
                "title": clean_text(row.get("caseName") or row.get("caseNameFull") or "CourtListener item"),
                "link": urllib.parse.urljoin("https://www.courtlistener.com", absolute_url) if absolute_url else source.url,
                "summary": clean_text(row.get("snippet") or row.get("plain_text") or ""),
                "published_kst": published.isoformat() if published else "",
            }
        )
    return items
...
def parse_link_html(text: str, source: Source) -> list[dict]:
    ...
    for match in link_pattern.finditer(text):
        ...
        if len(title) < 8 or not any(keyword in title.lower() for keyword in keyword_pool):
            continue
        ...
        deduped[link] = {
            ...
            "title": title,
            "link": link,
            "summary": f"{source.name} official page link: {title}",
            "published_kst": "",
        }
...
def parse_kind_html(text: str, source: Source, now: dt.datetime) -> list[dict]:
    ...
    row_pattern = re.compile(
        r"(?P<date>\d{2}\.\d{2})\s*\([^)]+\)\s*(?P<time>\d{2}:\d{2}).{0,120}?(?P<report>단일판매|공급계약|유상증자|전환사채|자기주식|합병|최대주주|투자판단|소송).{0,120}?",
        re.S,
    )
    items = []
        ...
        items.append(
            {
                ...
                "title": title,
                "link": source.url,
                "summary": f"KRX KIND disclosure candidate: {title}",
        ...
        items.append(
            {
                ...
                "title": f"{ticker} {company_name} filed {form}{': ' + description if description else ''}",
                "link": f"https://www.sec.gov/Archives/edgar/data/{int(cik)}/{accession_path}/{doc}",
                "summary": f"SEC EDGAR form {form}; accession {accession}; primary document {doc}",
                "published_kst": (parse_date(str(dates[idx])).isoformat() if parse_date(str(dates[idx])) else ""),
            }
        )
    return items[:10]
...
def collect_dart_filings(now: dt.datetime) -> tuple[list[dict], list[str]]:
    ...
    for row in rows:
        ...
        items.append(
            {
                "source": "OpenDART latest disclosures",
                "title": f"{corp_name} {report}",
                "link": link,
                "summary": f"DART stock_code={stock_code or 'N/A'} receipt={receipt_no or 'N/A'} rm={clean_text(row.get('rm')) or 'N/A'}",
                "published_kst": published.isoformat() if published else "",
            }
        )
...
def classify_item(item: dict) -> dict | None:
    ...
    matched = {
        bucket: [kw for kw in keywords if kw.lower() in haystack]
    ...
    sectors = [sector for sector, keywords in SECTOR_KEYWORDS.items() if any(kw.lower() in haystack for kw in keywords)]
    if not sectors:
        sectors = ["정책/규제 일반"]
    ...
    return {
        **item,
        ...
        "status": "예비" if item["source"].startswith(("CourtListener", "KRX KIND")) else "확정",
...
def render_report(alerts: list[dict], source_notes: list[str], now: dt.datetime) -> str:
    lines = [f"🚨 GAMEJOA 정책·규제 고충격 워치 · {now:%Y년 %m월 %d일 %H:%M KST}", ""]
    if not alerts:
        lines.extend(
            [
    ...
    for idx, alert in enumerate(alerts, 1):
        ...
        lines.extend(
            [
                f"## {idx}. [{alert['importance']}·{alert['status']}] {alert['title']}",
...
def write_outputs(alerts: list[dict], source_notes: list[str], now: dt.datetime) -> None:
    ...
    if alerts:
        top = alerts[0]
        (OUT_DIR / "gamejoa_policy_watch_alert_title.txt").write_text(
            f"GAMEJOA 정책 워치: [{top['importance']}] {top['title'][:70]}\n",
            encoding="utf-8",
        )
        (OUT_DIR / "gamejoa_policy_watch_alert.md").write_text(report, encoding="utf-8")
        (OUT_DIR / "gamejoa_policy_watch_alerts.json").write_text(
