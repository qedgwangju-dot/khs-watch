...
def _evidence_lines(groups) -> list[str]:
    ...
    for group in sorted(groups, key=lambda g: float(g.get("latest_epoch") or 0), reverse=True):
        for item in list(group.get("evidence") or []):
            source = str(getattr(item, "source", "주요 매체") or "주요 매체")
...
def _market_line(quotes) -> str | None:
    ...
    if brent is None:
        return None
    try:
        return f"• <b>Brent</b> {html.escape(core.format_quote(brent))}"
    except Exception:
        try:
            return f"• <b>Brent</b> {float(brent.price):,.2f}달러/배럴"
        except Exception:
            return None
...
def _build_bypass_body(groups, quotes) -> str:
    ...
    lines = [
        ...
        "",
        "<b>핵심 숫자</b>",
        "• <b>최대 수송능력</b> Aramco 공식 Q1 2026 · <b>700만배럴/일</b>",
        "• <b>전쟁 중 우회수송</b> Reuters 기준 약 <b>400만배럴/일</b>",
        "• <b>얀부 재고 버퍼</b> 송유관 중단 지속 시 약 <b>5~7일</b> 내 소진 가능 · Reuters 2026-09-13 추정",
        "• <b>최대 공급 위험</b> 장기 중단 시 약 <b>세계 원유 공급 4%</b> 추가 위험 · Reuters 추정",
    ]
    ...
    lines.extend([
        ...
        "• <b>2차 병목</b> 송유관이 복구돼도 Bab el-Mandeb·홍해 통항이 막히면 서쪽 우회효과가 제한됨",
        ...
        "",
        "<b>투자 포인트</b>",
        "• <b>돈버는능력</b> 비호르무즈 원유 공급원·정제마진 수혜 가능성 vs 원유 수입기업·항공·운송 원가 부담",
    ...
    lines.extend([
        "",
        "<b>다음 확인</b>",
        "• 수리 예상기간 → 실제 일일 수송량 → 얀부 재고 → Bab el-Mandeb·홍해 통항 → Brent·디젤 → 인플레이션·금리",
        "",
        "<b>핵심 한 줄</b>",
...
def build_regular_alert_v28(groups, quotes, new_signals, cleared_signals):
    ...
    metadata["hormuz_bypass_watch"]["readability"] = "dedicated oil-bypass body; generic Qatar/LNG impact text suppressed"
