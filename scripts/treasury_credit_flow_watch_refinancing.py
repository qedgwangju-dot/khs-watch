...
MSPD_URL = "https://api.fiscaldata.treasury.gov/services/api/fiscal_service/v1/debt/mspd/mspd_table_3_market"
...
def _get_rows(params):
    r = requests.get(MSPD_URL, params=params, headers=app.base.HEADERS, timeout=(8, 30))
    r.raise_for_status()
    payload = r.json()
    rows = payload.get("data") or []
    if not rows:
        raise RuntimeError("Treasury MSPD marketable table returned no rows")
    return rows


def get_refinancing_snapshot():
    latest_row = _get_rows({"sort": "-record_date", "page[number]": 1, "page[size]": 1})[0]
    ...
    next12_bills_t = to_tril(next12_bills_mil)
    next12_coupon_t = max(0.0, next12_t - next12_bills_t)
    bill_share = bills_t / total_t * 100.0 if total_t else None
    next12_share = next12_t / total_t * 100.0 if total_t else None

    # Annualized simple sensitivity if outstanding bills reprice by the stated rate move.
    sensitivity_b = {
...
def _refi_block(text):
    ...
    elif snap["next12_share"] is not None and snap["next12_share"] >= 20:
        state = "주의 — 향후 12개월 차환 비중 높음"
    else:
        state = "관찰 — 차환벽 상시 점검"
    ...
    return "\n".join([
        f"차환·재정 민감도: {state}",
        f"향후 12개월 만기: ${snap['next12_t']:.2f}T | 시장성 국채의 {snap['next12_share']:.1f}% | Bills ${snap['next12_bills_t']:.2f}T / 쿠폰물·기타 ${snap['next12_coupon_t']:.2f}T",
        ...
        f"연도별 기존 만기: 2027 ${snap['y2027_t']:.2f}T | 2028 ${snap['y2028_t']:.2f}T",
        "→ Bills는 짧게 반복 차환돼 Fed 금리 변화가 이자비용에 빨리 반영. 쿠폰물은 만기 시점에만 재가격되므로 모든 부채가 즉시 같은 금리로 바뀌는 것은 아님.",
        "→ 민감도는 현재 Bills 잔액에 금리 변화폭을 단순 적용한 연율 추정이며 실제 현금 이자비용은 재발행 시점·만기구성·낙찰금리에 따라 달라짐.",
        f"자료 기준: U.S. Treasury MSPD {snap['record_date']} (월말 공식 잔액)",
    ])
