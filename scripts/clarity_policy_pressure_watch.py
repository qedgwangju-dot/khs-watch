...
ACTION_RE = re.compile(
    ...
    r"advance[sd]?|pass(?:es|ed|age)?|vote|voting|motion\s+to\s+proceed|remain\s+at\s+the\s+negotiating\s+table|"
...
def fetch_bytes(url, timeout=15):
    req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 (KHS-CLARITY-Policy-Watch/1.3)"})
...
def korean_event(item, actor, event_type, source_label, signal):
    if event_type == "핵심 사업자·업계 표결 촉구":
        company = INDUSTRY_ACTORS.get(actor, "미국 암호자산 사업자")
        ko_title = f"{actor}, CLARITY 법안 표결·통과 촉구"
        detail = (
            f"{actor}이(가) 상원에 CLARITY 법안의 실제 표결·통과를 촉구했습니다. 직접 규제 대상 사업자의 이해관계가 있는 발언이므로 "
        ...
        detail = (
            "Trump 대통령이 CLARITY 법안의 통과·절차 진행을 공개적으로 압박한 새 발언이 신뢰 매체에서 확인됐습니다. 실제 표결이나 법률 효력 발생은 아니지만 상원 표결 시간표와 협상 압력을 바꿀 수 있는 정책 신호입니다."
        )
        ...
        detail = (
            "Bessent 재무장관이 CLARITY 법안의 통과·절차 진행을 공개적으로 압박한 새 발언이 신뢰 매체에서 확인됐습니다. 실제 표결이나 법률 효력 발생은 아니지만 상원 표결 시간표와 협상 압력을 바꿀 수 있는 정책 신호입니다."
        )
        ...
        detail = (
            "CLARITY 법안 자체 조문 외에 대통령·정부 관계자의 암호자산 이해관계와 윤리 조항이 상원 표 확보의 변수로 부각됐습니다. 이는 현재 매출보다 통과 확률과 시간표를 바꾸는 정치적 변수입니다."
        )
    else:
        company = ""
        mobility = False
        ...
        detail = (
            f"{actor or '미국 고위 정책 당사자'}가 CLARITY 법안의 통과·절차 진행을 공개적으로 압박한 새 발언이 신뢰 매체에서 확인됐습니다. 실제 표결이나 법률 효력 발생은 아니지만 상원 표결 시간표와 협상 압력을 바꿀 수 있는 정책 신호입니다."
        )
...
def main():
    ...
    for candidate in accepted:
        ...
        if baseline or sig in old_seen:
            continue
        new_events.append(korean_event(item, candidate["actor"], candidate["event_type"], candidate["source_label"], candidate["signal"]))

    merged = list(dict.fromkeys(list(state.get("seen_signatures") or []) + current))[-700:]
