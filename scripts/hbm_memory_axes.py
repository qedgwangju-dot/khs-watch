...
def comparison(old, new):
    ...
    if new['axis'] == 'hbm_product_mix':
        reasons = []
        old_target = float(a.get('target_pct') or 0)
        new_target = float(b.get('target_pct') or 0)
        if abs(new_target - old_target) >= 5:
            reasons.append(f"HBM4계열 목표 비중 {new_target-old_target:+.1f}%p")
        old_ref = float(a.get('reference_pct') or 0)
        new_ref = float(b.get('reference_pct') or 0)
        if abs(new_ref - old_ref) >= 5:
            reasons.append(f"기준 비중 {new_ref-old_ref:+.1f}%p")
        return reasons
...
def render(change, rate=None):
    ...
    names = {'rdimm_quote': '서버 RDIMM 공개 현물가격', 'ddr5_chip_quote': 'DDR5 16Gb 칩 현물가격', 'rdimm': '서버 DDR5 가격·프리미엄', 'hbm_config': 'HBM 칩 용량·적층 구성',
             ...
             'hbm_product_mix': '삼성 HBM4·HBM4E 제품 비중 변화'}
    def fmt(record):
        v = record['value']
        ...
        if record['axis'] == 'hbm_product_mix':
            return f"HBM4·HBM4E 비중 {v['reference_pct']:.0f}%→{v['target_pct']:.0f}%"
    ...
    if r['axis'] == 'hbm_product_mix':
        lines.append('• 제품 비중은 생산·출하 방향 지표이며 회사 전체 HBM 매출 점유율과 동일하지 않습니다.')
    ...
    display_title = korean_source_title(
        {'title': r.get('source_title',''), 'source': r.get('source','')},
        names.get(r['axis'], 'HBM 상태 변화'))
    lines += [f'• 근거 단계: {label}', '• 근거 제목(한국어): ' + html.escape(display_title) +
...
, '', title).strip()
    ...
    if 'samsung' in low and 'hbm4' in low and ('double' in low or '2x' in low):
        if ('product mix' in low or 'mix' in low) and len(pcts) >= 2:
            return f"보도: 삼성전자, 내년 HBM4·HBM4E 생산 2배 확대 가능성 — 제품 비중 {pcts[0]}%→{pcts[1]}%"
        return '보도: 삼성전자, 내년 HBM4·HBM4E 생산 2배 확대 가능성'
    if 'bernstein' in low and 'hbm' in low:
        return 'Bernstein HBM 매출·출하 추정 변화 관련 보도'
    if 'malaysia' in low and 'hbm' in low:
        return '말레이시아 HBM·첨단패키징 변화 관련 보도'
    return fallback + ' 관련 보도'


def parse_hbm_product_mix(item, body):
    title = item.get('title','')
    text = re.sub(r'\s+', ' ', title + ' ' + body)
    if not re.search(r'HBM4|HBM4E|HBM\s*4', text, re.I):
        return []
    if not re.search(r'product\s*mix|제품\s*비중|비중', text, re.I):
        return []
    ...
    if not m:
        m = re.search(
            r'([0-9]+(?:\.[0-9]+)?)\s*%[^.]{0,70}?'
            r'(?:product\s*mix|제품\s*비중|비중)[^.]{0,70}?'
            r'([0-9]+(?:\.[0-9]+)?)\s*%',
            text, re.I)
    if not m:
        return []
    ...
    if not period:
        period = published[:4] if re.match(r'^20\d{2}', published) else 'unknown'
    value = {'reference_pct': old_pct, 'target_pct': target_pct, 'product_family': 'HBM4/HBM4E'}
    return [make_record(
        'hbm_product_mix', ['samsung','HBM4_family',period], value, 'pct', period,
        item, title, scope='reported_product_mix_share_not_shipment_share')]
