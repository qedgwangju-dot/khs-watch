#!/usr/bin/env python3
"""Korea physical-AI infrastructure and ROBOTIS excess-demand lanes.

Pohang/Neuromeka: MOU -> fund/financing -> permits/build -> data/AI infra ->
external OEM orders -> utilization/revenue.
Hyundai Motor: robot data-factory plan -> site/robot count/equipment -> operation
-> measured field performance -> external deployment.
ROBOTIS: backlog/capacity imbalance -> Uzbekistan ramp -> yield/output ->
shipments/ASP -> additional OEM orders.

Guardrails keep plans, analyst estimates and capacity separate from secured
...
def topic_group(text):
    if _is_pohang_foundry(text): return 'pohang_robot_foundry'
    if _is_hyundai_data_factory(text): return 'hyundai_robot_data_factory'
    if _is_robotis_demand(text): return 'robotis_excess_demand'
...
'데이터 파운드리·추론 인프라 구축':'2단계 로봇 현장 데이터 파운드리와 추론 전용 AI 인프라가 제조 수수료 외 데이터·컴퓨팅 반복매출로 이어지는지 확인합니다.',
'로봇 파운드리 위탁생산·가동률':'뉴로메카 자체 로봇 생산을 넘어 외부 OEM 위탁생산 고객·수주·가동률이 붙는 시점이 실제 매출 검증 구간입니다.',
'포항 피지컬AI 로봇 파운드리':'로봇 제조·실증·데이터를 한 장소에 묶는 계획입니다. 금액보다 금융확정→착공→외부고객→가동률 순으로 실제 사업화를 봅니다.',
...
'외부 데이터 업체·계약':'기밀 제조데이터는 내부, 범용 데이터는 외부 조달하는 구조가 구체화되는 신호입니다. 업체 실명은 계약·공식자료 확인 때만 고객으로 확정합니다.',
'로봇 데이터 성능·현장 검증':'데이터 양이 실제 작업 성공률·사람 개입률·가동률 개선으로 바뀌는지 보는 핵심 구간입니다.',
...
'데이터 파운드리·추론 인프라 구축':'설비를 먼저 깔고 외부 고객이 늦게 붙으면 저가동률과 운영비가 먼저 발생할 수 있습니다.',
'로봇 파운드리 위탁생산·가동률':'외부 OEM 고객이 없으면 자체 생산공장에 그칩니다. 고객 실명·계약물량·가동률이 없으면 반복매출로 보지 않습니다.',
'포항 피지컬AI 로봇 파운드리':'1,800억원 전체를 이미 확정·집행된 예산으로 표시하지 않습니다. 단계별 자금확정과 착공을 따로 확인합니다.',
'데이터 팩토리 부지·로봇 대수':'서울 근교·수백대는 현재 보도 관측입니다. 현대차 확정 전에는 확정 수치로 알림하지 않습니다.',
'데이터 팩토리 설비·가동':'로봇 가동률·안전·정비시간이 낮으면 데이터 생산성이 떨어집니다. 발열·고장·사람 개입률을 함께 봅니다.',
'외부 데이터 업체·계약':'기사 언급만으로 현대차 고객으로 확정하지 않습니다. 제조 공정 데이터 보안과 외부 반출 제한도 병목입니다.',
'로봇 데이터 성능·현장 검증':'데이터 양 증가가 작업 일반화·성공률·실패복구 개선으로 이어지지 않으면 비용센터에 머물 수 있습니다.',
'현대차 로봇 데이터 팩토리':'시설 구축 계획은 공식이지만 위치·규모·로봇 대수·일부 일정은 미확정입니다.',
...
def meaning(cat):
    raw = cat.split(' · ',1)[-1]
    return _MEANING[raw] if raw in _MEANING else _orig_meaning(cat)
def risk(cat):
    raw = cat.split(' · ',1)[-1]
    return _RISK[raw] if raw in _RISK else _orig_risk(cat)

def verification(item, group, text):
    src = item.get('source') or ''
    if group == 'pohang_robot_foundry':
        return '당사자 공식자료 · 단계별 자금확정/착공 여부 확인' if src in {'포항시','경상북도','뉴로메카','Neuromeka'} else '신뢰 매체 보도 · 포항시·경상북도·뉴로메카 공식자료 교차확인'
    if group == 'hyundai_robot_data_factory':
        if src in {'현대자동차','현대자동차그룹','Hyundai Motor','Hyundai Motor Group'}: return '현대자동차그룹 공식자료'
        if SITE_SCALE.search(text): return '보도 단계 · 현대차 공식 계획은 확인, 부지·규모·로봇 대수는 추가확인 필요'
        return '신뢰 매체 보도 · 현대자동차그룹 공식자료 교차확인'
    if group == 'robotis_excess_demand':
        if src in {'로보티즈','ROBOTIS'}: return '로보티즈 공식자료'
        if ANALYST_ONLY.search(text) or re.search(r'초과\s*수요|excess\s*demand',text,re.I): return '증권사 분석 · 수주잔고·생산능력·출하 숫자 별도 검증'
...
def _same_event(a,b):
    ...
    if g == 'pohang_robot_foundry':
        ...
        for sig in [r'지역\s*활성화\s*투자\s*펀드|지역활성화투자펀드|금융\s*약정',r'착공|groundbreaking',r'위탁\s*생산|contract\s*manufacturing|OEM']:
            if re.search(sig,ta,re.I) and re.search(sig,tb,re.I) and (_numbers(ta)==_numbers(tb) or not _numbers(ta) or not _numbers(tb)): return True
    elif g == 'hyundai_robot_data_factory':
        current=r'서울\s*근교|수도권|수백\s*대|hundreds?\s+of\s+robots|데이터\s*팩토리|data\s*factory'; future=r'부지\s*확정|site\s*confirmed|설비\s*발주|equipment\s*(?:order|award)|착공|groundbreaking|가동\s*개시|operation\s*start'
        if re.search(current,ta,re.I) and re.search(current,tb,re.I) and not (re.search(future,ta,re.I) or re.search(future,tb,re.I)): return True
    elif g == 'robotis_excess_demand':
        if re.search(r'초과\s*수요|excess\s*demand',ta,re.I) and re.search(r'초과\s*수요|excess\s*demand',tb,re.I): return True
        # Same Microduck/Reachy supply thesis across syndicated rewrites; a new
        # numeric order/sales/take-rate value remains a separate event.
        if HUGGINGFACE.search(ta) and HUGGINGFACE.search(tb) and ACTUATOR.search(ta) and ACTUATOR.search(tb):
