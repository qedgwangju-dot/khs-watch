...
def render_report(alerts: list[dict], now: dt.datetime) -> str:
    ...
    if not alerts:
        lines.extend([
            ...
            "",
            "💡 워치 판단: 이번 실행에서 매출·마진·현금흐름, 밸류에이션/할인율, 수급, 시간표를 새로 바꾼 확정 이벤트는 직접 확인되지 않았습니다.",
            "",
    ...
    lines.extend([
        "💡 워치 판단: 이번 실행은 매출·마진·현금흐름, 밸류에이션/할인율, 수급, 시간표 중 실제로 바뀐 축을 기준으로 정책/규제 상태 변화 후보를 감지했습니다. 장전 레이더에서 원문 전문과 시장 반응을 재확인해야 합니다.",
        "",
        "투자 조언이 아닌 참고용 정책·규제 알림입니다.",
    ])
    return "\n".join(lines) + "\n"
