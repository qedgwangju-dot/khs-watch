...
def cross_series(usdjpy: list[tuple[float, float]], usdtarget: list[tuple[float, float]]) -> list[tuple[float, float]]:
...
def append_context(body: str, context: dict) -> str:
    ...
    lines += ["", "출처", "- USD/JPY: Yahoo query1/query2 5분 데이터 교차확인(USD/MXN·USD/BRL·USD/ZAR 조합, 지연 가능)"]
    return body.rstrip() + "\n\n" + "\n".join(lines) + "\n"
...
def create_alert(now: dt.datetime, pending: dict, context: dict, reason: str) -> None:
    ...
    lines = [
        f"조회 시각: {now.astimezone(KST).strftime('%Y-%m-%d %H:%M:%S KST')}", "", "판정",
        f"- 캐리 청산 위험: {unwind_label}",
        f"- 엔화 재약세·캐리 재구축: {rebuild_label}",
        "※ 두 판정은 서로 다른 질문이며 동시에 높거나 서로 엇갈릴 수 있습니다.", "", "이번 변화", f"- {reason}",
    ]
    body = append_context("\n".join(lines), context)
    ...
    payload = {
        "verdict": {
            "unwind_level": int(pending.get("unwind_level") or 0), "unwind_label": unwind_label,
            "rebuild_level": int(pending.get("rebuild_level") or 0), "rebuild_label": rebuild_label,
            "evidence": pending.get("evidence") or {},
        },
        "reasons": [reason], "errors": [], "target_currency_confirmation": context,
...
def process(now: dt.datetime | None = None) -> int:
    ...
    if not incomplete:
        ...
        # Missing data are unknown, not a live confirmation. Exclude them from the current risk score.
        ch15 = ch30 = ch60 = 0.0
    ...
    md = ["# 엔캐리 투자통화 확산 확인", f"- 조회시각(KST): {context['checked_at_kst']}", f"- 확인 통화: {available}/3",
    ...
    pending = {
        "initialized": True, "updated_at_kst": context["checked_at_kst"],
        "active_confirmation": bool(classification.get("active_confirmation")),
        "stressed_count": int(classification.get("stressed_count") or 0),
        "stressed_codes": list(classification.get("stressed_codes") or []), "available_count": available,
    }
    if incomplete and previous.get("initialized"):
        # Preserve the last confirmed state for persistence only, while the current context remains excluded from scoring.
        pending["active_confirmation"] = bool(previous.get("active_confirmation"))
        pending["stressed_count"] = int(previous.get("stressed_count") or 0)
        pending["stressed_codes"] = list(previous.get("stressed_codes") or [])
    ...
    if previous.get("initialized") and not incomplete:
        old_active, new_active = bool(previous.get("active_confirmation")), bool(pending["active_confirmation"])
        old_count, new_count = int(previous.get("stressed_count") or 0), int(pending["stressed_count"])
    ...
    if ALERT_BODY.exists():
        ALERT_BODY.write_text(append_context(ALERT_BODY.read_text(encoding="utf-8"), context).rstrip() + "\n", encoding="utf-8")
    ...
    print(json.dumps({"active_confirmation": classification.get("active_confirmation"), "stressed_count": classification.get("stressed_count"),
                      "available_count": available, "alert_reason": reason, "errors": len(errors)}, ensure_ascii=False))
    return 0
