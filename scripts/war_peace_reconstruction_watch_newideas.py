...
def _newidea_signals(row):
    ...
    if any(k in text for k in (
        ...
        signals.append('우크라이나·미국·유럽이 후속 3자 회담을 준비하는 단계로 이동')
        marks.append('3자회담준비')
...
def newidea_score_item(x, now):
    ...
    if marks:
        ...
        if any(k in src for k in ('reuters','president of ukraine','president.gov.ua','연합뉴스','yonhap')):
            score += 10
...
def newidea_item_id(x):
    ...
    if not marks:
        return base_id
    # 제안→3자회담 준비→의제/후속회동처럼 협상 단계가 진전되면 후속 알림을 허용한다.
    important = [m for m in marks if m in (
        '합리적평화제안','새종전아이디어','3자회담준비','회담후보지','실무의제',
        '포로교환','트럼프후속회동','겨울방공지원','돌파구없음','조기종전신중',
    )]
    key = base_id + '|newideas-v2|' + '|'.join(important)
...
def _inject_newideas(text, items):
    ...
    if '3자회담준비' in marks:
        rows.append('- <b>현재 단계:</b> 후속 3자 회담 준비 단계 — 개최 확정·합의 문안은 아직 별개')
    if '회담후보지' in marks:
